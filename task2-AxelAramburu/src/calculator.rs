...
impl Calculator {
    ...
    pub fn show_history(&self) -> String{
        ...
        for (op_index, operation) in self.history.iter().enumerate() {
            let res = operation.operation_type.perform(operation.first_num, operation.second_num).unwrap_or_default();
            history_str.push_str(&format!("{}: {} {} {} = {}\n", op_index, operation.first_num, operation.operation_type.get_sign(), operation.second_num, res));
        }
        history_str
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        ...
        op.operation_type.perform(op.first_num, op.second_num)
    }
