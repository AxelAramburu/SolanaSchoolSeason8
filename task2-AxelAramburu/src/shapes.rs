...
impl Rectangle {
    pub fn new(width: f64, height: f64) -> Result<Self, Error> {
        ...
        } else if height.is_sign_negative() {
...
impl Shape for Rectangle {
    ...
    fn perimeter(&self) -> f64 {
        let res = (self.height + self.width) * 2.0;
        res
    }
}
...
impl Shape for Circle {
    fn area(&self) -> f64 {
        let res = std::f64::consts::PI * self.radius * self.radius;
        res
    }
    fn perimeter(&self) -> f64 {
        let res = 2.0 * std::f64::consts::PI * self.radius;
        res
    }
}